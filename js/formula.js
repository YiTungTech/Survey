...
function calculate_B_typeA(qmconfig) {
    ...
    $.each(qmconfig.B_typeA.session, function(i, item_session) {
        ...
        // console.log('「生活型態建議」排序後=' + JSON.stringify(tempLifeArray));//「生活型態建議」排序後=[{"qid":"299261106","title":"練習「以口吸氣，鼻用力呼氣」的深呼吸運動","sort":2},{"qid":"1848259859","title":"多利用抽風機、電風扇幫助空間換氣","sort":9},{"qid":"1221090849","title":"增加氧源，栽植室內盆栽","sort":10}]
        ...
        if (isNeedExercise) {
            ...
            $.each(item_session.exercise, function(eIndex, item_exercise) {
                tempExerciseArray.push(item_exercise);
            });
            // console.log('「運動處方建議」排序前 tempExerciseArray=' + JSON.stringify(tempExerciseArray));
            ...
            tempExerciseArray.sort(function(a, b) {
                function cmp(x, y) {
                    return x > y ? 1 : (x < y ? -1 : 0); //等於的時候不要動
                }
            ...
            // console.log('「運動處方建議」 排序後=' + JSON.stringify(tempExerciseArray));//「運動處方建議」 排序後=[{"qidGroup":[{"qid":"1848259859"},{"qid":"1221090849"}],"title":"加強鍛鍊胸肌、腹肌、上背肌等呼吸肌群。","sort":3},{"qidGroup":[{"qid":"299261106"}],"title":"進行脊椎、豎脊肌伸展運動。","sort":1}]
        }
